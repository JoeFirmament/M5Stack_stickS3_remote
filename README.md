...
- 顶部 `J` 徽标显示 Joystick2 状态：红色实心代表已连接，灰色空心代表未连接或已进入外设省电状态。
- 可单独使用 StickS3 的 A/B 键。
- 可选 Unit Joystick2；启动时通过 Grove I²C 自动检测，不需要刷不同固件。
- 配置和 Reader 键位映射保存在 NVS，重启后继续使用。
- 内置平衡型省电策略：自动熄屏、BMI270 拿起唤醒、蓝牙 modem sleep、CPU 动态降频和 Joystick2 空闲断电。
...
Joystick2 默认地址为 `0x63`。启动时固件会提前打开 Grove 5V，并以短间隔重试检测，给模块内置 STM32 留出启动时间；未检测到模块时会关闭外部输出并继续使用 A/B 双键界面。
...
- 20 秒无操作后 LCD 自动休眠；按 A/B、拿起 StickS3，或操作仍在供电的 Joystick2，都会立即唤醒屏幕。
- BMI270 仅启用 25Hz 低功耗加速度计，固件在熄屏时以 10Hz 检测整机位姿变化；陀螺仪、温度和辅助传感器保持关闭。当前唤醒阈值为 `0.32g`，并要求连续两次采样达到阈值，只响应明显的拿起、晃动或翻转，过滤桌面轻碰和线缆扰动。
- Joystick2 的 RGB 指示灯默认关闭。
- Joystick2 操作期间以 20ms 间隔读取，空闲时自动降为 100ms。
- 5 分钟无操作后关闭 Grove 5V，令 Joystick2 完全断电，此时顶部 `J` 变为灰色。拿起整台遥控器或按 A/B 会恢复模块供电；恢复成功后 `J` 重新变红。Joystick2 断电后，原装四线 Grove 接口不能独立感知纯摇杆位移，因此需要带动 StickS3 本体产生动作。
- 从外设断电状态唤醒时，固件先恢复 Grove 5V、等待 Joystick2 MCU 稳定，再点亮 LCD，以免两个负载的启动电流叠加并触发 Brownout。
- 为保持蓝牙连接，固件不进入会断开 BLE 的 Deep Sleep；拿起唤醒依靠 BMI270，而 Reset 键不参与日常唤醒。
- ESP32-S3 在空闲期间动态降频，BLE 控制器在无线事件之间使用 modem sleep。
- 串口每 60 秒输出一次电量、电池电压、屏幕和 Joystick2 供电状态，便于检查续航异常。
...
- 浏览器视频快退/快进需要播放器获得键盘焦点；先点击一次视频画面，再推动摇杆左/右。
...
- 继续测量并优化真实待机电流，提供可选的“始终待命”和“最长续航”模式。
