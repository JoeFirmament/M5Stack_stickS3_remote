...
- 内置平衡型省电策略：自动熄屏、蓝牙 modem sleep、CPU 动态降频和 Joystick2 空闲断电。
...
- 20 秒无操作后 LCD 自动休眠；按 A/B 或操作仍在供电的 Joystick2 会立即唤醒屏幕。
- Joystick2 的 RGB 指示灯默认关闭。
- Joystick2 操作期间以 20ms 间隔读取，空闲时自动降为 100ms。
- 5 分钟无操作后关闭 Grove 5V，令 Joystick2 完全断电，此时顶部 `J` 变为灰色。由于 Grove 接口没有独立中断线，此后需要先按 A 或 B 恢复模块供电；恢复成功后 `J` 重新变红，仅推动摇杆无法唤醒。
